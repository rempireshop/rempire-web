 ...
 * on an order that is already paid, and a guard on every call would silently
 * refuse it.
 */
 ...
 *
 * The caller is the transition into `paid` (src/lib/payments/apply.ts), the
 * same race claimOrderPaid() above answers for the door that goes through
 * settle.ts: the shopper's return and Montonio's webhook race by design, both
 * arrive holding an order that said «не оплачен», and a plain UPDATE lets both
 * of them believe they are the one that moved it — so the gift card is spent
...
export async function setOrderStatus(
  ...
  /* …and the day «Отправлен» was pressed, written the same way and for the
     same reason. The auto-«Доставлен» clock in src/lib/delivery.ts counts N
     days from the hand-over, and `updated_at` is not that day: every carrier
     status webhook writes saveShipmentOnOrder(), which sets `updated_at =
     now()` (src/lib/shipping/montonio.ts), so a parcel that reports progress
     more often than `autoDays` pushed its own deadline forward for ever and
     never closed. Written once, like deliveredAt: an undo back to `paid` and a
     second «Отправлен» keep the first date, because the parcel left once. An
     order shipped before this existed has no key, and delivery.ts falls back
     to `updated_at` for it exactly as it always did. */
  ...
  /* inventory, the other way: the journal's undo of a cancellation or a
     ...
     src/lib/payments/apply.ts, which does its own decrement, and a second one
     here would take the quantity off the shelf twice. That per-line ledger
     test is also what stands in for isTracked(): a line with a 'return' move
     of ours is a line move() counted, and an untracked variant has none. */
