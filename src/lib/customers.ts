 ...
 *
 * `address` is the courier's, and only the courier's (Dim, 23.09.2026:
 * «everything set on the account → buy in 3 taps» — a courier default
 * without a door to ring could never be one). It is the checkout's own three
 * fields, in the order's own words (`shipping.address` — {addr, zip, city}),
 * cleaned the way createOrder() cleans them (shipText in src/lib/orders.ts:
 * control characters out, spaces folded, 160 characters), and kept only
 * whole: the checkout will not take a courier order short of any of the
 * three, so half an address is not something to fill in or to buy with.
 * Absent — not empty — otherwise, so every preference stored before it reads
 * exactly as it did.
 */
