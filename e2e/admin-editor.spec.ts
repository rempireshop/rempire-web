...
test.describe("admin — the product editor", () => {
  ...
  test("one page, no «Сохранить», and what is typed reaches the shop and the warehouse", async ({ page, browser }) => {
    ...
    try {
      ...
      await expect(row, "«Склад» does not show the barcode the card bound").toContainText(ean);
      // 1a: «Склад» shows the count in the box between − and +
      await expect(row.locator("[data-stockqtyinput]"), "«Склад» does not show the count the card set").toHaveValue(String(stockWas + 7));
