...
type PostOut = { offline?: boolean; noApi?: boolean; body?: unknown; status?: number };
...
describe("postJSON tells «no shop here» from «no answer»", () => {
  it("404, 405 and 501 are the static host — and only they set noApi", async () => {
    for (const status of [404, 405, 501]) {
      const out = await post(() => answer(status, { ok: true }));
      expect(out.noApi, `status ${status}`).toBe(true);
      expect(out.offline).toBe(true);
    }
  });

  it("a dead connection is offline but NOT noApi", async () => {
    const out = await post(() => Promise.reject(new TypeError("Failed to fetch")));
    expect(out.offline).toBe(true);
    expect(out.noApi).toBeUndefined();
  });

  it("an answer that is not JSON — a gateway's own error page — is offline but NOT noApi", async () => {
...
describe("payNow never calls a real order a demonstration", () => {
  ...
  it("a lost answer to POST /api/orders/ is an error, not the demo receipt", async () => {
    ...
    expect(run.toasts).toEqual(["Связь прервалась — проверьте интернет и попробуйте ещё раз"]);
  ...
  it("a lost answer to POST /api/payments/create/ is an error too", async () => {
    const run = await payNow([{ body: { ok: true, orderId: "o1", number: "R-1" }, status: 201 }, { offline: true }]);
    expect(run.demo).toBe(0);
    expect(run.toasts).toEqual(["Связь прервалась — проверьте интернет и попробуйте ещё раз"]);
  ...
  it("404 from the orders route IS the static prototype — the demo still walks", async () => {
    const run = await payNow([{ offline: true, noApi: true }]);
  ...
  it("a failed background probe does not put the live checkout into demo mode", async () => {
    const run = await payNow([{ body: { ok: true, orderId: "o1", number: "R-1" }, status: 201 }, { offline: true }], false);
    expect(run.demo).toBe(0);
...
describe("«Сообщить о наличии» does not thank people it never signed up", () => {
  ...
  it("404 is still the prototype's demo «Записали ✓»", async () => {
    const out = await notifySend({ offline: true, noApi: true });
