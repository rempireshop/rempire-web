...
describe("postJSON() — «there is no API here» versus «it did not answer»", () => {
  ...
  it("says failed — never offline — when the connection dropped", async () => {
    expect(await postJSON(dropped)).toEqual({ failed: true });
  });

  it("says failed for an edge 502/504 whose body is an HTML error page", async () => {
    for (const status of [500, 502, 503, 504]) {
      expect([status, await postJSON(html(status))]).toEqual([status, { failed: true }]);
    }
  });
