...
function routes(): RouteCase[] {
  ...
  return [
    ...
    { name: "GET /api/admin/promos/", path: "/api/admin/promos/", method: "GET", exports: ["GET", "POST", "PATCH", "DELETE"], load: () => import("@/app/api/admin/promos/route"), auth: "admin", req: admin },
    { name: "POST /api/admin/promos/", path: "/api/admin/promos/", method: "POST", exports: ["GET", "POST", "PATCH", "DELETE"], load: () => import("@/app/api/admin/promos/route"), auth: "admin", req: admin, body: { code: "FUZZ20", kind: "percent", value: 20, minSubtotal: 0, startsAt: null, endsAt: null, maxUses: 10, active: true, note: "n" } },
    { name: "PATCH /api/admin/promos/", path: "/api/admin/promos/", method: "PATCH", exports: ["GET", "POST", "PATCH", "DELETE"], load: () => import("@/app/api/admin/promos/route"), auth: "admin", req: admin, body: { code: "FUZZ10", active: false } },
    // round 15: a code nobody has used can be deleted — the code rides in the
    // query string, so the junk this fuzzes is the same junk the bundles
    // DELETE above gets (a code that is on an order answers 409, never 5xx)
    { name: "DELETE /api/admin/promos/", path: "/api/admin/promos/", method: "DELETE", exports: ["GET", "POST", "PATCH", "DELETE"], load: () => import("@/app/api/admin/promos/route"), auth: "admin", req: admin, queries: ["?code=FUZZ20", "?code=", "?code=%00", "?code=%2e%2e%2f", "?code=NOSUCHCODE"] },
    ...
    // «Запустить сейчас» — runs the daily job's own function for one letter (src/lib/flows.ts runFlowByHand); every switch is off in this suite, so nothing is mailed
