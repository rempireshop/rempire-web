...
const CONFIRM_CARDS: Array<[string, string]> = [
  ...
  // …and a part of it, typed into the box (map defect 9)
  ["refund, a part",
    `${ORDER}\nВернём 10 € из 34,90 € через Montonio — тем же путём, каким деньги пришли. Клиенту уйдёт письмо, статус заказа и склад не изменятся.`],
  ["refund, a part of what is left",
    `${ORDER}\nПо заказу уже возвращено 10 €. Вернём ещё 5 € из оставшихся 24,90 € через Montonio — тем же путём, каким деньги пришли. Клиенту уйдёт письмо, статус заказа и склад не изменятся.`],
  ...
  // …and either split after an earlier refund (order-refund-retry e4)
  ["refund split with a gift card, after an earlier refund",
    `${ORDER}\nПо заказу уже возвращено 4,50 €, осталось 30,40 €.\nВернём на подарочную карту: 30,40 € · на счёт покупателя: 0 €\nКартой снова можно будет платить. Клиенту уйдёт письмо.`],
