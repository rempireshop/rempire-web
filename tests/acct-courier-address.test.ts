 ...
 *
 * Dim, 23.09.2026: «In Account, the user who always uses courier should still
 * have the option to set a default/standard address.» A courier default had
 * no door to ring — the account kept {country, method, carrier, machine} and
 * nothing else — so a customer who gets his parcels to the door typed his
 * street into every checkout. (It was first added the same day for the
 * express buy, and went with it when that was taken out; it is back on its
 * own, with nothing of the express.)
 *
 ...
 * cleans an order's address, and put into the checkout's boxes by
 * applyAcctShipPref().
 *
