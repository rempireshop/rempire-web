...
describe("the webhook and the shopper's return race each other", () => {
  ...
  it("claimOrderPaid answers true to exactly one caller", async () => {
    const order = await createOrder(giftOrder);
    expect(await claimOrderPaid(order.id, "payment:mock")).toBe(true);
    expect(await claimOrderPaid(order.id, "payment:mock")).toBe(false);
