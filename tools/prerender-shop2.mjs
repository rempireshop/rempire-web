...
/* The viewport meta is the second thing in the hand-maintained head this
   tool writes (VIEWPORT_META, src/lib/seo-head.mjs): it is one line every
   page must agree on, so it is patched into the shell here rather than typed
   into index.html by hand — the same idea as the token above. */
const shell = reviewport(retokenise(shellOnDisk, ASSET_V));
