...
- **Google Merchant feeds** `/feed/google-{en,et,ru}.xml`
  (`src/app/feed/[file]/route.ts` over `src/lib/merchant-feed.ts`), built per
  request from the live catalogue — overrides, per-size stock, the checkout's
  shipping rules — with photos and texts from `src/data/catalogue.feed.json`
  (`tools/pack-feed.mjs`, in `prebuild`). Links always on rempireshop.com.
  Owner's steps: `docs/merchant-feed.md`.
