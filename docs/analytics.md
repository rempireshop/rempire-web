...
| **Из корзины в заказ** | Sessions that bought ÷ sessions that opened the shop — "out of every 100 people who opened the shop, this many bought something." BOTH sides are the same consent-gated `events` stream and the same session id, which is what makes that sentence true; the numerator is the subset of the denominator that went on to buy, so the figure cannot exceed 100 %. Until 17.09.2026 the numerator was instead every PAID ORDER, with no channel filter: salon sales rung up at the till counted as people who "came into the shop", while the denominator only ever saw shoppers who had accepted the statistics banner. Both errors pushed the same way and the card could read far above the truth, and above 100 %. Correcting it made the number FALL — that is the fix, not a regression. It now measures only what the shop can actually see: it counts no till sale and nobody who declined analytics. Called «Конверсия» before the redesign. |
...
| `GET /api/admin/overview/` | requireAdmin. Everything on the «Обзор» tab: `{orders:{today,yesterday}, revenue7d:{total,perDay,orders}, revenueByDay, lowStock:{total,low,out,items}, attention:{ordersToShip,proRequests,reviewsPending,stockAlerts,returnRequests}}`. No parameters — the windows are fixed (today, yesterday, the last 7×24 h). `no-store`; 503 `db_unavailable` when there is no database. `revenueByDay` is the screen's own seven bars, built by the same `qRevenueByDay()` the "7d" range of `/api/admin/analytics` uses: until 13.09.2026 the Overview fetched that whole range — sixteen queries — for this one field, and it now waits on this call alone. |
