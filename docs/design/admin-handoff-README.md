...
- **Assistant FAB**: 52 h ink, icon + «Помощник» (icon only on phone), bottom-right 20/28 px; hidden while the assistant pane/sheet is open or in scanner mode. On a phone, while a save bar is on screen (`body.adm-saving`), the FAB is HIDDEN — the bar is the screen's header then (see «Товар (editor)») and the button would sit on the form; the assistant is a row of the «Ещё» sheet instead (r13, 11.09.2026). Desktop pane: 380 px right column, header 64 h with «›» collapse button; phone: 75 %-height sheet.
...
### Товар (editor) — fix #2
«← Товары»; kicker «Бренд · Раздел» (or «Новый товар»); title; «Открыть в магазине ↗». Tabs: **Основное · Размеры и цены · Фото и видео · Описание · Google**. **Sticky save bar** at the bottom on every tab (desktop and tablet, ≥ 768): «Сохранить» primary · «Отмена». The destructive verb is NOT in the bar on any width (r13, 11.09.2026): «Снять с продажи» — like «Удалить набор» in the set editor and «Удалить письмо» in the newsletter — ends the form on its own hairline row (`.adm-danger`) as a quiet warn-coloured link, behind its confirm card as before (undo restores). **On a phone (≤ 767) the bar IS the screen's top header** (Dim, 11.09.2026 — on the nav it took a quarter of an iPhone 14's screen): fixed at the top, one 52-px row plus the notch inset, paper over a hairline, the iPhone idiom of Notes/Contacts — «Отмена» (or «Отменить правки») as quiet text on the left, ONE status word in the middle («Не сохранено» warn / «Сохранено ✓» ok for 2–3 s / «Изменений нет» muted; ellipsis, never a second row), the primary on the right (a 40-px ink block inside a 44-px target; a ring instead of ink while nothing differs) — and the panel's own header (logo, languages, «← В магазин») is hidden while it stands. A refusal («Цена — число от 1 до 500 €…») is a second row UNDER the header, full width, warn, only while it stands. The page's top padding and `scroll-padding-top` clear the bar's measured height (`--a-hdrh`, a ResizeObserver in renderImpl), so a focused field never lands under it; the bar follows iOS's visual viewport when the keyboard pushes it (`--a-vvtop`); the bottom padding is the nav's again, the toast returns above the nav, and the assistant's FAB is hidden (the assistant is in «Ещё»). The same on EVERY screen that saves, so the panel behaves one way throughout. The same `.adm-savebar` is the pattern for every form that saves: the promo form, the set editor, «Новый партнёр», the mail editor (10.09.2026) and — since r12 — every settings page with a draft (Главная страница, О компании, Доставка и оплата, Цены и баллы): ONE bar per page, the only «Сохранить» on it, with a status line (`.adm-savebar__note`) that names the unsaved card — «Изменения не сохранены: Главный баннер» — «Изменений нет» plus a disabled ghost button while nothing differs, «Сохранено ✓» on the button once the change went through, and «Отменить правки» beside it; the cards keep only their reset links. On a tablet the status line is its own row above the buttons; on a desktop it follows them; on a phone the header's middle word says just «Не сохранено» (the full sentence is hidden there). The bars that had no status line (the product editor, a promo, a set, a partner, the mail texts, a newsletter) carry the same word on the phone: «Не сохранено» from the first keystroke (`admBarTouch`, keyed on the open form) or from the form's own dirty check (`mailDirty()`, `newsDirty()`), «Сохранено ✓» for 2–3 s after a save that leaves the form open. The bar is painted in place on every keystroke (never a render() — the caret). A single field with its own button (an order's or a customer's note, «Неоплаченные заказы») follows the same three states in place: quiet/disabled → ink + «Не сохранено» → «Сохранено ✓» on the button; Enter is the button, Ctrl+Enter in the note's textarea. A stock row's «Править» form keeps a live «Сохранить» (four boxes, the stepper beside them): an untouched form folds away with «Изменений нет» instead of closing in silence, the button says «Сохраняем…» during the round trip, and the row it wrote carries a «Сохранено ✓» badge until it is edited or stepped again. The blog's «Публикация» card carries its own line: «Ещё не сохранено» / «Есть несохранённые изменения» / «Сохранено ✓».
...
- Размеры и цены: grid `1.2fr 1fr 1fr .8fr 1.4fr 44px` — Размер · Цена € · Салон € (auto = price×0.8 on price change, editable) · Остаток (warn ≤ 3) · Штрихкод (mono, placeholder «не привязан», a 44-px «Сканер» button beside it — icon only on desktop — that opens the scanner for THAT size; the code read lands in that box and is bound by «Сохранить», like a typed one) · × remove (min 1 row). Phone: the size and its × share the first line, the barcode line spans the whole row (a 13-digit EAN, the button and «Отвязать» need it). «+ Размер» dashed button.
- Фото и видео: photo grid `minmax(120px,1fr)`, first tagged «главное» (ink), others tagged with the size name; ← → ★ ✂ × per photo in a strip that wraps inside its tile; dashed «+ Фото с телефона». Phone (≤ 899): one column, and a photo is a ROW — 72-px thumb · tag · one line of 44-px buttons that belongs to that photo alone (a 160-px tile cannot hold five of them, 10.09.2026); the add button is a dashed 64-h bar. Hint: first photo main, drag to reorder, watermark automatic. Video: chips **YouTube · Instagram · Загрузить** (fix #12); link input (mono) for the first two, dashed 64-h upload button for the third («MP4 до 200 МБ»); attached video row with ▶ thumb and ×.
...
### Сканер — fix #7
Full-screen dark mode (`#1c1a00`), header «Сканер» · mode caption («Склад: приёмка и списание» / «Продажа: товар добавится в корзину» / «Товар: код встанет в поле «Штрихкод»» — the third is the editor's door: opened from a size's «Сканер» button, the first code read goes into that size's box, nothing is looked up or written, the overlay closes itself and the editor is as it was; the manual button reads «Вписать» there) · × (44). Idle: 320-px viewfinder square with warn scan line, hint, manual-code input (mono). Found: white card — «Найдено · EAN» (ok), product 600 17, «size · на складе N», giant stepper (64-px buttons, Oswald 48 qty), «Принять +N» primary / «Списать −N» secondary (in POS mode: single action adds to cart and returns), «Сканировать дальше». Unknown code: warn «Код не привязан · code», «К какому товару?», autofocused search, candidate list (product · size, «без кода» marker) — one tap binds EAN to that size, toast, then goes straight to the Found state. Footer note: same screen ships as the standalone admin-only «Rempire Сканер» PWA. Three taps max: scan → (bind) → quantity → Принять.
...
### Настройки — fix #6
Index list (max 640): 6 rows 64 h with label + muted description + chevron. Sub-pages have «← Настройки» + title, and every page with a draft ends in the sticky save bar described under «Товар (editor)» — the phone's top header there (r13) — one «Сохранить» per page, a status line naming the unsaved card (one word, «Не сохранено», on the phone), «Отменить правки»; the cards themselves carry no save button (r12). Banner editor: «Добавить слайд» disables at five slides with the reason beside it («Максимум 5 слайдов — удалите один, чтобы добавить новый»); picking a picture or a link target patches the form in place, so the page never moves (app.js `paintHeroPick`).
- Доставка и оплата: tariff grid Страна · Пакомат € · Курьер € · Бесплатно от € (rows Эстония, Латвия, Литва, Финляндия) · «Сохранить» (the page's bar; goes through confirm card) · «Заполнить по тарифам Montonio»; then Оплата switches (карта/банк-ссылки, Apple/Google Pay, при получении).
