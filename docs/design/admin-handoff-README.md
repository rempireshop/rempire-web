...
- **Assistant FAB**: 52 h ink, icon + «Помощник» (icon only on phone), bottom-right 20/28 px; hidden while the assistant pane/sheet is open or in scanner mode. On a phone, while a save bar is on screen (`body.adm-saving`), the FAB is 48 × 48 and sits INSIDE the bar's row in its right-hand corner — the bar reserves 56 px for it — never over the form and never over the nav (10.09.2026). Desktop pane: 380 px right column, header 64 h with «›» collapse button; phone: 75 %-height sheet.
...
### Товар (editor) — fix #2
«← Товары»; kicker «Бренд · Раздел» (or «Новый товар»); title; «Открыть в магазине ↗». Tabs: **Основное · Размеры и цены · Фото и видео · Описание · Google**. **Sticky save bar** at the bottom on every tab: «Сохранить» primary · «Отмена» · destructive «Удалить» (confirm card; undo restores). On a phone the bar stands ON the bottom nav — offset by the nav's drawn height, safe-area inset included (`--a-barh`, admin.css) — the page's bottom padding shrinks to the nav so the bar never comes unstuck at the end of the scroll, and the toast climbs above the bar. The same `.adm-savebar` is the pattern for every form that saves: the promo form, the set editor, «Новый партнёр», the mail editor (10.09.2026) and — since r12 — every settings page with a draft (Главная страница, О компании, Доставка и оплата, Цены и баллы): ONE bar per page, the only «Сохранить» on it, with a status line (`.adm-savebar__note`) that names the unsaved card — «Изменения не сохранены: Главный баннер» — «Изменений нет» plus a disabled ghost button while nothing differs, «Сохранено ✓» on the button once the change went through, and «Отменить правки» beside it; the cards keep only their reset links. On a phone the status line is its own row above the buttons; on a desktop it follows them. The bar is painted in place on every keystroke (never a render() — the caret). A single field with its own button (an order's or a customer's note, «Неоплаченные заказы») follows the same three states in place: quiet/disabled → ink + «Не сохранено» → «Сохранено ✓» on the button; Enter is the button, Ctrl+Enter in the note's textarea. A stock row's «Править» form keeps a live «Сохранить» (four boxes, the stepper beside them): an untouched form folds away with «Изменений нет» instead of closing in silence, the button says «Сохраняем…» during the round trip, and the row it wrote carries a «Сохранено ✓» badge until it is edited or stepped again. The blog's «Публикация» card carries its own line: «Ещё не сохранено» / «Есть несохранённые изменения» / «Сохранено ✓».
