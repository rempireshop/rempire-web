...
| D C | **Check the live account without money**: put the live pair in `.env.montonio-live` (gitignored), run `node --env-file=.env.montonio-live tools/montonio-live-check.mjs` and, for prices, `tools/fetch-montonio-tariffs.mjs --dry` with the same file; then delete the file. GET requests only. It says which payments and banks are on per country, whether every carrier the checkout offers can be booked, and whether the parcel webhook is registered. | to do |
| D | **Register the parcel webhook**: Shipping → Webhooks → `https://rempireshop.diipsolutions.eu/api/shipping/notify/` — trailing slash, or the POST is lost in a 308. Events `shipment.registered`, `shipment.registrationFailed`, `shipment.statusUpdated`. Re-point it at rempireshop.com when the domain moves. | to do |
