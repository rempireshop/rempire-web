...
## Re-audit, 04.09.2026 — the surfaces added in the last 24 hours

Branch `q-security` from `64f66c1`. Scope: the owner's own products
(`src/lib/custom-products.ts`, `/api/admin/products`, the `custom` block of
`GET /api/overrides`, `priceItems()`), the request-time product page and the
...
uploads and «Убрать фон» (`/api/admin/upload`, `/api/admin/upload/cutout`,
`src/lib/photo-cutout.ts`, `src/lib/storage.ts`), the assistant's new actions
(`src/app/api/assistant/actions.ts` — the route itself was exercised, not
edited), the blog snippet generator (`/api/admin/ai/text`, kind `post`), the
per-language Google pair (`src/lib/product-seo.ts`, both routes that write it)
and the header block in `next.config.ts`. Method: read, write a failing vitest
case for every hole, fix, extend the fuzz registry. Every finding below has a
test in `tests/security-*.test.ts` (4 files, 35 cases); the whole suite is
63 files / 1 061 tests, `npm run typecheck` clean, and the headers were read
off a running `next dev` for the route-handler responses.

### Fixed

| # | Severity | What was wrong | Where / fix |
| --- | --- | --- | --- |
| **R1** | Medium | **Stored markup injection through the JSON-LD of a custom product's page.** `headBlock()` put `JSON.stringify(o)` raw inside `<script type="application/ld+json">`. `line()` in `custom-products.ts` strips control and format characters only, so a brand or name carrying `</script>` (or `<!--`) was stored as typed and ended the block on a public page; everything after it was parsed as markup — for the RU page, the name is also the `<h1>`, but that one was escaped. The `/shop2/*` CSP (`script-src 'self'`) stops inline script and handlers, `form-action` and `frame-src` stop a fake form or frame, so the reach was defacement and a `<meta http-equiv=refresh>` — Medium, not High; the author is the owner, or an assistant proposal he confirms without reading. | `src/lib/seo-head.mjs` `ldJson()`: `<`, `>`, U+2028 and U+2029 leave as `\u` escapes — the same JSON to every reader. The prerender uses the same function; the catalogue carries none of those characters, so its output is byte-identical. |
| **R2** | Medium | **A tampered size label bought the big bottle at the small bottle's price.** `priceItems()` took an unknown `variant` on a product with a size ladder — `"250 ml"` against a `"250 мл"` ladder, an index past the end — as a free-text label and priced the line at the base (first) price, so the owner shipped what the label said at 14,90 instead of 24,90. Catalogue products too, not only `c-…` ids. | `src/lib/orders.ts` `priceItems()`: with a ladder to check against, an unknown size is `bad_variant` (400). A product with no ladder keeps taking the cart's label at its one price, as before. |
| **R3** | Medium | **`PUT /api/admin/overrides` stored unbounded, untyped values that `/api/overrides` then served to the world, cached at the edge, with no delete path** (audit L8, left open): `id` of any length, `seoTitle`/`seoDesc`/`subcat` of any type and size, `varImg` as any JSON. And the `description` alias for `seoDesc` took the trilingual `{RU,ET,EN}` object too, writing it into `seo_desc` as JSON text — which `getSeoOverrides()` read back as the Russian Google description. | `src/app/api/admin/overrides/route.ts` `normalise()`: `id` ≤ 120, no whitespace or control character (`bad_id`); `seoTitle`/`seoDesc` one clean line at 70/170 like the per-language pair; `subcat` a string ≤ 40; `varImg` ≤ 32 integers in −1…999; anything else `bad_body`. `description` is the `seoDesc` alias only when it is a string — so `{description: null}` no longer wipes `seo_desc` as a side effect (flagged as a behaviour correction, see below). |
| **R4** | Low | **No deadline on «Убрать фон».** Neither the model call in `cutoutImage()` nor the fetch of the original had a timeout: a hung upstream held the function open until the platform killed it. The upstream error body (which can quote the request) was kept in the error message. | `src/lib/photo-cutout.ts`: `AbortSignal.timeout(cutoutTimeoutMs())` — 90 s, `PHOTO_CUTOUT_TIMEOUT_MS` overrides; only the upstream status is kept. `upload/cutout/route.ts`: the source fetch gets the same deadline and answers `source_unavailable`; a `CutoutError` is logged by code and status, never with the key or the request. |
| **R5** | Low | `DELETE /api/admin/upload?key=giftcards/…` removed a printable gift card the shop wrote for itself (`isAllowedKey()` is the coarse "one of ours" gate and knows that prefix); the letter already sent then pointed at nothing. | `upload/route.ts` DELETE: `giftcards/` is refused — only what an upload can create can be deleted. |
| **R6** | Low | `POST /api/admin/ai/text` parsed a body of any size (Vercel's 4,5 MB) before the per-field caps in `ai-prompts.ts` applied. | `ai/text/route.ts`: the body is read as text and refused past 128 KB (413 `too_large`), the same door `/api/orders` uses. |
| **R7** | Low | `set_seo`, `stock_adjust` and `stock_set` sliced their strings but did not clean them — control characters and newlines rode through into the panel and, for `set_seo`, into a `<title>`. | `actions.ts` `oneLine()`: control/format characters gone, whitespace collapsed, capped (70/170 for the pair, 120 for a variant). |
| **R8** | Low | `cleanSeoPatch()` kept control and format characters in the Google pair — both routes that write it, and a custom product's own `seo`. | `src/lib/product-seo.ts` `line()` strips `\p{Cc}` and `\p{Cf}` like every other one-line cleaner in the repo. |
| **R9** | Low | The storefront toast wrote its message into `innerHTML` unescaped, and «Добавлено: ‹name› · ‹size›» carries a custom product's name and size as the owner typed them (app.js `paintToast()`). CSP stops script; it was markup by the owner into his own shop. | `public/shop2/app.js` `paintToast()`: `esc()` on the message, shop and admin toast alike (no caller passes markup). |
| **R10** | Low | `cleanCustomProductInput()` read and cleaned every size label before counting them — a list of a hundred thousand labels cost a hundred thousand regex passes before `too_many_sizes`. | `custom-products.ts`: the count is checked first. |
| — | — | **Fuzz registry.** The three product-page routes and the sitemap live outside `src/app/api`, so the "every route on disk" check never saw them and they were not fuzzed; `/api/admin/products/[id]` was fuzzed with an order id as its "good" id. | `tests/fuzz-routes.test.ts`: the four routes added, the walk covers all of `src/app`, `goodIdFor()` hands the product routes a real `c-…` id from a new `customId` fixture (`tests/fuzz-harness.ts`). |

### Checked and left as it is

- **Custom products.** `isCustomId()` (`^c-[a-z0-9][a-z0-9-]*$`, ≤ 80) gates every read and write by id; the routes answer 404 to anything else. `cleanCustomProductInput()` rebuilds the row field by field — a JSON body's `__proto__` is an own property nobody reads (pinned) — with sizes and prices aligned, prices in 1–500 €, descriptions through `cleanDescriptionPatch()` (4 000), photos through `cleanGallery()`, the pair through `cleanSeoPatch()`. `GET /api/overrides` publishes active rows only, in `toCatalogueProduct()` shape (no pro price, no hidden row). `priceItems()` ignores every number the browser sends; a hidden row answers `out_of_stock`, an unknown or wrong-case id `unknown_item`, an override on a `c-…` id applies exactly as on a catalogue id. `customers.ts` (alerts, cart snapshot) and `analytics.ts` resolve names from active rows only.
- **The page.** Every HTML text and attribute goes through `esc()`; the `<title>` is safe because `<` is escaped; the shell path is a constant (`SHELL_REL`) — no id reaches the file system; hidden → the shell as a 404 with `noindex, nofollow` and `no-store`; a database outage → the shell, `no-store`, never a 5xx; a shown page is `s-maxage=60, stale-while-revalidate=300`. That last one means a product hidden while its page is cached stays visible at the edge for up to a minute — the same window the overrides feed has, and the checkout refuses it regardless. Judged acceptable.
- **The sitemap.** Ids are slugs, every URL is escaped, `lastmod` comes from an ISO stamp; `s-maxage=300`; an outage is an empty, valid urlset with `no-store`.
- **Uploads.** Kind whitelist; the owner id through `safeId()` (lower-cased slug, no `..`, ≤ 80 — so `C-Upper` is accepted as `c-upper`, by design); the size checked before and after the read; the type from magic bytes (`sniffImage`/`sniffVideo`), never from the name or the Content-Type; sharp with `limitInputPixels`, metadata stripped; 60 uploads an hour per session. Cut-out: the key must be `products/…` with an image extension, a URL must start with our public base, `putObject()` re-checks `isAllowedKey()`, 40 an hour per session, the answer is sniffed as PNG before it is stored. R2 objects are public by design — product photos — with `cache-control: immutable`; there is no ACL to get wrong.
- **Assistant.** `moderate_review` and `set_product_active` are journal entries the panel writes for itself (`demoApply()` in app.js), not model actions: `sanitizeAction()` returns null for both, pinned. `create_product` mirrors the library's bounds (checked against each other in `tests/custom-products.test.ts`). The admin gate — cookie **and** Origin — was exercised through the route with the new action: shop mode never carries it, admin mode without either answers 401/403.
- **The blog snippet.** The task comes from `AI_TASKS` on the server, every input field is capped in `ai-prompts.ts` (title 200, excerpt 500, body 1 500, 12 tags, 12 products), the system prompt is fixed and the article lands under `INPUT:` in the user turn, the answer is shaped to `{title ≤ 70, description ≤ 170}` and nothing else; 30 calls an hour. An article that argues with the prompt can at worst produce a bad snippet in the owner's own editor.
- **Headers.** Read off `next dev` for `/shop2/p/<id>/` and `/shop2/et/p/<id>/`: the route-handler responses carry the strict `/shop2/*` policy — `script-src 'self' https://static.cloudflareinsights.com`, no `unsafe-inline`, no `unsafe-eval`; `frame-ancestors 'none'`; `X-Frame-Options: DENY`; HSTS one year with subdomains. The inline `<style>html{background:#fff}</style>` and `#prestyle` are covered by `style-src 'unsafe-inline'`, which was already load-bearing (app.js writes `style=` attributes). No new `unsafe-*` anywhere; the only `'unsafe-inline'` in a `script-src` is on `/:path*` (Next's own flight payload) and the mail preview frame, as before — pinned by `tests/security-product-page.test.ts`.

### Still open

- **`cleanGallery()`** (`src/lib/orders.ts`, backend-core) accepts a protocol-relative `//host/x.png` and any `https://` host, so an owner-typed gallery URL can point at a third party (`img-src https:` allows it). Admin-only and harmless to shoppers beyond a third-party image request; left with its owner.
- **`maxDuration`** on `/api/admin/upload/cutout`: the route exports none, so on a plan whose default function timeout is below the model's answer time the platform kills the call before the 90 s deadline does. Set it once the Vercel plan is settled (the value that is legal differs by plan).
- **Behaviour correction to note for the panel:** `PUT /api/admin/overrides` with `{description: null}` used to clear `seo_desc` as a side effect of the alias; it now clears the trilingual description only. Nothing in app.js relied on the side effect.
- The admin toast at `app.js` («Снято с продажи · …») and the journal's `actionText()` strings reach the DOM through `esc()` at their sinks (`adm-jrow__x`, the confirm and propose cards) — checked, no change; the toast sink itself is escaped now (R9).
- **M3/M6** from the first audit (per-instance limiters, 30-day sessions) are unchanged; the new limiters (uploads, cut-outs) key on the session cookie's tail, which is the right key for a one-owner shop and the same best-effort kind as the rest.
