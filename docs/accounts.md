...
| Railway | Production hosting: Next.js app + Postgres + volume (uploads) | rempireshopinfo@ (login via GitHub) | Renat | Dim via GitHub collaborator | Hobby $5/mo (usage inside credit) | ⬜ | EU region; card: Dim's temporarily |
...
| DATABASE_SSL_NO_VERIFY | optional, **security-shaped**. `1` connects to the database without checking its certificate. Since 07.09 this is the only thing that turns verification off — the host-name exemption for `*.rlwy.net` / `*.railway.app` was removed (docs/backend.md, docs/audit/2026-09-07-cleanup.md). If a deploy's `postbuild` migrate starts failing with `SELF_SIGNED_CERT_IN_CHAIN`, that is Railway's self-signed certificate and this is the answer; if it does not, production TLS is now genuinely verified and this should stay unset | Dim (in Vercel) | ⬜ not set — watch the first deploy after 07.09 |
